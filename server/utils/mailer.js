...
export const sendOtpEmail = async (to, otp, name) => {
  const isPlaceholder = !process.env.RESEND_API_KEY || process.env.RESEND_API_KEY === "your_resend_api_key";

  if (isPlaceholder) {
    console.warn("⚠️  [MAILER] Using mock email mode because RESEND_API_KEY in server/.env is not configured.");
  ...
  try {
    ...
    const { error } = await resend.emails.send({
      from: "UniThrift <onboarding@resend.dev>",
      to: [to],
